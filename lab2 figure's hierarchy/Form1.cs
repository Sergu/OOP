...
namespace oop1_figuer_s_hierarchy
{
    public partial class Form1 : Form
    {
        ...
        ICreator creator;
        List<IShapeDraw> drawlist;
        IShapeDraw figuredraw;
        List<Point> points;
        bool drawFlag;
        ...
        private void StackDraw(List<IShapeDraw> stack)
        {
            foreach (IShapeDraw currentobj in stack)
            {
                currentobj.Draw(graph);
            }
        }

        public void but_AllDelete_Click(object sender, EventArgs e)
        {
            //IShapeDraw curobj = drawlist.Pop();
            drawlist.Clear();
            objcount = 0;
            Label_CountOfObjects.Text = Convert.ToString(objcount);
            graph.Dispose();
        ...
        //private void But_TopDelete_Click(object sender, EventArgs e)
        //{
        //    //try
        //    //{
        //        drawlist.RemoveAt(Convert.ToInt32(Text_ID.Text));
        //        graph.Dispose();
        //        this.Refresh();
        //        graph = CreateGraphics();
        //        StackDraw(drawlist);
